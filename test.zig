...
pub fn main() !void {
    ...
    var alice_keys = rsa.gen_keys();
    var t2 = std.time.milliTimestamp();
    std.debug.print("TIME: {any}\n", .{t2 - t1});
    std.debug.print("ALICE'S KEYS:\nPublic Key: {any}\nPrivate Key: {any}\n", .{ alice_keys.public_key.n, alice_keys.private_key.d });
    std.debug.print("SAVING ALICE'S KEYS TO PEM...\n", .{});
    try rsa.save_private_key(alice_keys, "private.pem");
    try rsa.save_public_key(alice_keys, "public.pem");
    std.debug.print("LOADING ALICE'S KEYS FROM PEM...\n", .{});
    alice_keys = try rsa.load_public_key("public.pem");
    alice_keys = try rsa.load_private_key("private.pem");
    std.debug.print("ALICE'S KEYS:\nPublic Key: {any}\nPrivate Key: {any}\n", .{ alice_keys.public_key.n, alice_keys.private_key.d });
    std.debug.print("GENERATING KEYS.....\n", .{});
    ...
    std.debug.print("TIME: {any}\n", .{t2 - t1});
    std.debug.print("BOB'S KEYS:\nPublic Key: {any}\nPrivate Key: {any}\n", .{ bob_keys.public_key.n, bob_keys.private_key.d });
    t1 = std.time.milliTimestamp();
    const alice_signature = rsa.sign("SIGNED DATA", alice_keys);
    t2 = std.time.milliTimestamp();
    std.debug.print("TIME: {any}\n", .{t2 - t1});
    ...
    t1 = std.time.milliTimestamp();
    const alice_to_bob_decrypted = rsa.decrypt(try alice_to_bob_encrypted, bob_keys);
    t2 = std.time.milliTimestamp();
    std.debug.print("TIME: {any}\n", .{t2 - t1});
